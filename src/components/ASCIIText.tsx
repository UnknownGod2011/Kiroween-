...
void main() {
    ...
    float waveFactor = uEnableWaves * 0.3;
...
void main() {
    ...
    float r = texture2D(uTexture, pos + cos(time * 1.5 - time + pos.x) * .005).r;
    float g = texture2D(uTexture, pos + tan(time * .3 + pos.x - time) * .005).g;
    float b = texture2D(uTexture, pos - cos(time * 1.5 + time + pos.y) * .005).b;
...
const ASCIIText = React.memo(function ASCIIText({
  text = 'AR TRY-ON',
  asciiFontSize = 12,
  textFontSize = 240,
  textColor = '#60a5fa',
    ...
    >
      <style>{`
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: linear-gradient(135deg, #60a5fa 0%, #a78bfa 50%, #60a5fa 100%);
